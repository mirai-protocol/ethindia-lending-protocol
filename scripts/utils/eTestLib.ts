...
async function buildContext(provider, wallets, tokenSetupName) {
  ...
  // Token Setup
  ctx.tokenSetup = import(`./token-setups/${tokenSetupName}`);

  ctx.activateMarket = async (tok) => {
    let result = await (
      await ctx.contracts.markets.activateMarket(
        ctx.contracts.tokens[tok].address
      )
    ).wait();
    if (process.env.GAS) console.log(`GAS(activateMarket) : ${result.gasUsed}`);
    let eTokenAddr = await ctx.contracts.markets.underlyingToEToken(
      ctx.contracts.tokens[tok].address
    );
    ctx.contracts.eTokens["e" + tok] = await ethers.getContractAt(
      "EToken",
      eTokenAddr
    );
    let dTokenAddr = await ctx.contracts.markets.eTokenToDToken(eTokenAddr);
    ctx.contracts.dTokens["d" + tok] = await ethers.getContractAt(
      "DToken",
      dTokenAddr
    );
  };
  ...
  ctx.populateUniswapPool = async (pair, fee) => {
    const addr = await ctx.contracts.uniswapV3Factory.getPool(
      ctx.contracts.tokens[pair[0]].address,
      ctx.contracts.tokens[pair[1]].address,
      fee
    );

    ctx.contracts.uniswapPools[`${pair[0]}/${pair[1]}`] =
      await ethers.getContractAt("MockUniswapV3Pool", addr);
    ctx.contracts.uniswapPools[`${pair[1]}/${pair[0]}`] =
      await ethers.getContractAt("MockUniswapV3Pool", addr);

    let inverted = ethers.BigNumber.from(
      ctx.contracts.tokens[pair[0]].address
    ).gt(ctx.contracts.tokens[pair[1]].address);
  ...
  ctx.createUniswapPool = async (pair, fee) => {
    await (
      await ctx.contracts.uniswapV3Factory.createPool(
        ctx.contracts.tokens[pair[0]].address,
        ctx.contracts.tokens[pair[1]].address,
        fee
      )
  ...
  ctx.encodeUniswapPath = async (
    ...
    let pools = await Promise.all(
      poolSymbols.map(async (ps) => {
        let [t0s, t1s] = ps.split("/");
        let t0 = new Token(
          1,
          ctx.contracts.tokens[t0s].address,
          await ctx.contracts.tokens[t0s].decimals(),
          t0s,
          "token0"
        );
        let t1 = new Token(
          1,
          ctx.contracts.tokens[t1s].address,
          await ctx.contracts.tokens[t1s].decimals(),
          t1s,
          "token1"
        );
        tokens[t0s] = t0;
  ...
  ctx.getUniswapInOutAmounts = async (
    amount,
    poolSymbols,
    liquidity,
    sqrtPriceX96 = ratioToSqrtPriceX96(1, 1),
    zeroForOne
  ) => {
    ...
    if (
      ctx.contracts.tokens[t0s].address.toLowerCase() >
      ctx.contracts.tokens[t1s].address.toLowerCase()
    )
      [t0, t1] = [t1, t0];
    ...
    let [outAmount] = await pool.getOutputAmount(
      CurrencyAmount.fromRawAmount(zeroForOne ? t1 : t0, amount)
    );
    let [inAmount] = await pool.getInputAmount(
      CurrencyAmount.fromRawAmount(zeroForOne ? t1 : t0, amount)
    );
    return {
  ...
  ctx.tokenBalancesSlot = async (token) => {
    ...
    if (ctx.tokenBalancesSlot[token] !== undefined)
      return ctx.tokenBalancesSlot[token];
    ...
    for (let i = 0; i < 100; i++) {
      let slot = ethers.utils.keccak256(
        module.exports.abiEncode(["address", "uint"], [account, i])
      );
      ...
      let prev = await network.provider.send("eth_getStorageAt", [
        address,
        slot,
        "latest",
      ]);
  ...
  ctx.setTokenBalanceInStorage = async (
    token,
    account,
    amount,
    balancesSlot
  ) => {
    if (balancesSlot === undefined)
      balancesSlot = await ctx.tokenBalancesSlot(token);

    return ctx.setStorageAt(
      ctx.contracts.tokens[token].address,
      ethers.utils.keccak256(
        module.exports.abiEncode(["address", "uint"], [account, balancesSlot])
      ),
      "0x" +
        module.exports
          .units(amount, await ctx.contracts.tokens[token].decimals())
          .toHexString()
          .slice(2)
          .padStart(64, "0")
    );
  ...
  ctx.doUniswapSwap = async (from, tok, dir, amount, priceLimit) => {
    ...
    if (
      ethers.BigNumber.from(ctx.contracts.tokens.WETH.address).gt(
        ctx.contracts.tokens[tok].address
      )
    ) {
      buy = !buy;
    ...
    if (buy) {
      let tx = await ctx.contracts.simpleUniswapPeriphery
        .connect(from)
        .swapExact0For1(
          ctx.contracts.uniswapPools[`${tok}/WETH`].address,
          amount,
          from.address,
          priceLimitRatio
        );
  ...
  ctx.setIRM = async (underlying, irm, resetParams) => {
    await (
      await ctx.contracts.governance
        .connect(ctx.wallet)
        .setIRM(underlying, irm, resetParams)
    ).wait();
  };
  ...
  ctx.setReserveFee = async (underlying, newReserveFee) => {
    await (
      await ctx.contracts.governance
        .connect(ctx.wallet)
        .setReserveFee(underlying, newReserveFee)
    ).wait();
  };

  ctx.setAssetConfig = async (underlying, newConfig) => {
    let config = await ctx.contracts.markets.underlyingToAssetConfigUnresolved(
      underlying
    );
    ...
    if (newConfig.borrowFactor !== undefined)
  ...
  ctx.decodeBatch = async (items, resp) => {
    ...
    for (let i = 0; i < resp.length; i++) {
      o.push(
        ctx
          ._batchItemToContract(items[i])
          .interface.decodeFunctionResult(items[i].method, resp[i].result)
      );
    }
  ...
  ctx.getContract = async (proxy) => {
    ...
    if (!cache[proxy]) {
      let [contractName, contract] =
        Object.entries(ctx.contracts).find(([, c]) => c.address === proxy) ||
        [];
...
function linearIRM(totalBorrows, poolSize) {
  ...
  let utilisation = et
    .eth(totalBorrows)
    .mul(et.c1e18.mul(2 ** 32 - 1))
    .div(total)
    .div(et.c1e18);
  return et
    .units("0.000000003168873850681143096", 27)
    .mul(utilisation)
    .div(2 ** 32 - 1);
}
...
function exportAddressManifest(ctx) {
  ...
  for (let swapHandlerName of Object.keys(ctx.contracts.swapHandlers)) {
    output.swapHandlers[swapHandlerName] =
      ctx.contracts.swapHandlers[swapHandlerName].address;
  }
...
async function deployContracts(provider, wallets, tokenSetupName) {
  ...
  if (ctx.tokenSetup.testing) {
    ...
    for (let token of ctx.tokenSetup.testing.tokens || []) {
      ctx.contracts.tokens[token.symbol] = await (
        await ctx.factories.TestERC20.deploy(
          token.name,
          token.symbol,
          token.decimals,
          false
      ...
      console.log(
        `Deployed: ${token.symbol} - ${
          ctx.contracts.tokens[token.symbol].address
        }`
      );
    }

    for (let [symbol, { address }] of Object.entries(
      ctx.tokenSetup.testing.forkTokens || {}
    )) {
      ctx.contracts.tokens[symbol] = await ethers.getContractAt(
        "TestERC20",
        address
      );
      console.log(`Using: ${symbol} - ${ctx.contracts.tokens[symbol].address}`);
    }
    ...
    if (ctx.tokenSetup.testing.useRealUniswap) {
      {
        ...
        ctx.uniswapV3FactoryFactory = new ethers.ContractFactory(
          abi,
          bytecode,
          ctx.wallet
        );
        ctx.contracts.uniswapV3Factory = await (
          await ctx.uniswapV3FactoryFactory.deploy()
        ).deployed();
        console.log(
          `Deployed: Uniswap V3 Factory - ${ctx.contracts.uniswapV3Factory.address}`
        );
      }
      {
        const {
        ...
        ctx.SwapRouterFactory = new ethers.ContractFactory(
          abi,
          bytecode,
          ctx.wallet
        );
        ctx.contracts.swapRouterV3 = await (
          await ctx.SwapRouterFactory.deploy(
            ctx.contracts.uniswapV3Factory.address,
            ctx.contracts.tokens["WETH"].address
          )
        ).deployed();
        console.log(
          `Deployed: SwapRouter V3 - ${ctx.contracts.swapRouterV3.address}`
        );
      }
      {
        const {
        ...
        ctx.SwapRouter02Factory = new ethers.ContractFactory(
          abi,
          bytecode,
          ctx.wallet
        );
        ctx.contracts.swapRouter02 = await (
          await ctx.SwapRouter02Factory.deploy(
        ...
        console.log(
          `Deployed: SwapRouter V2 - ${ctx.contracts.swapRouter02.address}`
        );
      }
      {
        const {
      ...
      ctx.contracts.uniswapV3Factory = await (
        await ctx.factories.MockUniswapV3Factory.deploy()
      ).deployed();
      ctx.uniswapV3PoolByteCodeHash = ethers.utils.keccak256(
        (await ethers.getContractFactory("MockUniswapV3Pool")).bytecode
      );
    }

    ctx.contracts.invariantChecker = await (
      await ctx.factories.InvariantChecker.deploy()
    ).deployed();
    console.log(
      `Deployed: InvriantChecker - ${ctx.contracts.invariantChecker.address}`
    );

    ctx.contracts.flashLoanNativeTest = await (
      await ctx.factories.FlashLoanNativeTest.deploy()
    ).deployed();
    console.log(
      `Deployed: FlashLoanNativeTest - ${ctx.contracts.flashLoanNativeTest.address}`
    );

    ctx.contracts.flashLoanAdaptorTest = await (
      await ctx.factories.FlashLoanAdaptorTest.deploy()
    ).deployed();
    console.log(
      `Deployed: FlashLoanAdaptorTest - ${ctx.contracts.flashLoanAdaptorTest.address}`
    );

    ctx.contracts.flashLoanAdaptorTest2 = await (
      await ctx.factories.FlashLoanAdaptorTest.deploy()
    ).deployed();
    console.log(
      `Deployed: FlashLoanAdaptorTest2 - ${ctx.contracts.flashLoanAdaptorTest2}`
    );

    ctx.contracts.simpleUniswapPeriphery = await (
      await ctx.factories.SimpleUniswapPeriphery.deploy()
    ).deployed();
    console.log(
      `Deployed: SimpleUniswapPeriphery - ${ctx.contracts.simpleUniswapPeriphery}`
    );
    ...
    if (ctx.tokenSetup.testing.useRealUniswap) {
      for (let tok of ctx.tokenSetup.testing.activated) {
        ...
        let config = ctx.tokenSetup.testing.tokens.find(
          (t) => t.symbol === tok
        );
        await (
          await ctx.contracts.uniswapPools[`${tok}/WETH`].initialize(
            ctx.poolAdjustedRatioToSqrtPriceX96(
              `${tok}/WETH`,
      ...
      console.log(`Initialised: UniswapPools`);
    }
  ...
  ctx.contracts.eulerGeneralView = await (
    await ctx.factories.EulerGeneralView.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: EulerGeneralView - ${ctx.contracts.eulerGeneralView.address}`
  );
  ...
  if (ctx.tokenSetup.existingContracts) {
    if (ctx.tokenSetup.existingContracts.swapRouterV2)
      swapRouterV2Address = ctx.tokenSetup.existingContracts.swapRouterV2;
    if (ctx.tokenSetup.existingContracts.swapRouterV3)
      swapRouterV3Address = ctx.tokenSetup.existingContracts.swapRouterV3;
    if (ctx.tokenSetup.existingContracts.swapRouter02)
      swapRouter02Address = ctx.tokenSetup.existingContracts.swapRouter02;
    if (ctx.tokenSetup.existingContracts.oneInch)
      oneInchAddress = ctx.tokenSetup.existingContracts.oneInch;
  }

  ctx.contracts.modules.installer = await (
    await ctx.factories.Installer.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE Installer - ${ctx.contracts.modules.installer.address}`
  );

  ctx.contracts.modules.markets = await (
    await ctx.factories.Markets.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE Markets - ${ctx.contracts.modules.markets.address}`
  );

  ctx.contracts.modules.liquidation = await (
    await ctx.factories.Liquidation.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE Liquidation - ${ctx.contracts.modules.liquidation.address}`
  );

  ctx.contracts.modules.governance = await (
    await ctx.factories.Governance.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE Governance - ${ctx.contracts.modules.governance.address}`
  );

  ctx.contracts.modules.exec = await (
    await ctx.factories.Exec.deploy(gitCommit)
  ).deployed();
  console.log(`Deployed: MODULE Exec - ${ctx.contracts.modules.exec.address}`);

  ctx.contracts.modules.swap = await (
    await ctx.factories.Swap.deploy(
      gitCommit,
      swapRouterV3Address,
      oneInchAddress
    )
  ).deployed();
  console.log(`Deployed: MODULE Swap - ${ctx.contracts.modules.swap.address}`);

  ctx.contracts.modules.swapHub = await (
    await ctx.factories.SwapHub.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE SwapHub - ${ctx.contracts.modules.swapHub.address}`
  );

  ctx.contracts.modules.eToken = await (
    await ctx.factories.EToken.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE eToken - ${ctx.contracts.modules.eToken.address}`
  );

  ctx.contracts.modules.dToken = await (
    await ctx.factories.DToken.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE dToken - ${ctx.contracts.modules.dToken.address}`
  );

  ctx.contracts.modules.riskManager = await (
    await ctx.factories.RiskManager.deploy(gitCommit, riskManagerSettings)
  ).deployed();
  console.log(
    `Deployed: MODULE RiskManager - ${ctx.contracts.modules.riskManager.address}`
  );

  ctx.contracts.modules.irmDefault = await (
    await ctx.factories.IRMDefault.deploy(gitCommit)
  ).deployed();
  console.log(
    `Deployed: MODULE irmDefault - ${ctx.contracts.modules.irmDefault.address}`
  );

  if (ctx.tokenSetup.testing) {
    ctx.contracts.modules.irmZero = await (
      await ctx.factories.IRMZero.deploy(gitCommit)
    ).deployed();
    ctx.contracts.modules.irmFixed = await (
      await ctx.factories.IRMFixed.deploy(gitCommit)
    ).deployed();
    ctx.contracts.modules.irmLinear = await (
      await ctx.factories.IRMLinear.deploy(gitCommit)
    ).deployed();

    if (ctx.tokenSetup.testing.forkTokens) {
      ctx.contracts.modules.irmClassLido = await (
        await ctx.factories.IRMClassLido.deploy(gitCommit)
      ).deployed();
    }
  }
  ...
  ctx.contracts.euler = await (
    await ctx.factories.Euler.deploy(
      ctx.wallet.address,
      ctx.contracts.modules.installer.address
    )
  ).deployed();
  console.log(`\nDeployed: EULER CONTRACT - ${ctx.contracts.euler.address}\n`);

  // Get reference to installer proxy

  ctx.contracts.installer = await ethers.getContractAt(
    "Installer",
...

  {
    ...
    if (ctx.tokenSetup.testing) {
      modulesToInstall.push("irmZero", "irmFixed", "irmLinear");
    ...
    let moduleAddrs = modulesToInstall.map(
      (m) => ctx.contracts.modules[m].address
    );

    await (
      await ctx.contracts.installer
        .connect(ctx.wallet)
        .installModules(moduleAddrs)
    ).wait();
  ...
  ctx.contracts.markets = await ethers.getContractAt(
    "Markets",
    await ctx.contracts.euler.moduleIdToProxy(moduleIds.MARKETS)
  );
  ctx.contracts.liquidation = await ethers.getContractAt(
    "Liquidation",
    await ctx.contracts.euler.moduleIdToProxy(moduleIds.LIQUIDATION)
  );
  ctx.contracts.governance = await ethers.getContractAt(
    "Governance",
    await ctx.contracts.euler.moduleIdToProxy(moduleIds.GOVERNANCE)
  );
  ctx.contracts.exec = await ethers.getContractAt(
    "Exec",
    await ctx.contracts.euler.moduleIdToProxy(moduleIds.EXEC)
  );
  ctx.contracts.swap = await ethers.getContractAt(
    "Swap",
    await ctx.contracts.euler.moduleIdToProxy(moduleIds.SWAP)
  );
  ctx.contracts.swapHub = await ethers.getContractAt(
    "SwapHub",
    await ctx.contracts.euler.moduleIdToProxy(moduleIds.SWAP_HUB)
  );

  // Deploy swap handlers
  ctx.contracts.swapHandlers.swapHandlerUniswapV3 = await (
    await ctx.factories.SwapHandlerUniswapV3.deploy(swapRouterV3Address)
  ).deployed();
  ctx.contracts.swapHandlers.swapHandler1Inch = await (
    await ctx.factories.SwapHandler1Inch.deploy(
      oneInchAddress,
      swapRouterV2Address,
      swapRouterV3Address
    )
  ).deployed();
  ctx.contracts.swapHandlers.swapHandlerUniAutoRouter = await (
    await ctx.factories.SwapHandlerUniAutoRouter.deploy(
      swapRouter02Address,
      swapRouterV2Address,
      swapRouterV3Address
    )
  ...
  if (ctx.tokenSetup.testing) {
    ...
    for (let tok of ctx.tokenSetup.testing.activated) {
      ...
      console.log(`\nActivated: Market - ${tok}\n`);
    }

    for (let tok of ctx.tokenSetup.testing.tokens || []) {
      if (tok.config) {
        if (!ctx.tokenSetup.testing.activated.find((s) => s === tok.symbol))
          throw `can't set config for unactivated asset: ${tok.symbol}`;
        await ctx.setAssetConfig(
          ctx.contracts.tokens[tok.symbol].address,
          tok.config
        );
        console.log(`\nCONFIG SET: Market - ${tok.symbol}\n`);
      }
    }
  }
  ...
  ctx.contracts.flashLoan = await (
    await ctx.factories.FlashLoan.deploy(
  ...
  console.log(
    `\nDeployed: Flashloan adaptor - ${ctx.contracts.flashLoan.address}\n`
  );

  // Setup liquidity mining contracts

  if (ctx.contracts.tokens.EUL) {
    ctx.contracts.eulStakes = await (
      await ctx.factories.EulStakes.deploy(ctx.contracts.tokens.EUL.address)
    ).deployed();

    console.log(
      `\nDeployed: Eul Staker - ${ctx.contracts.eulStakes.address}\n`
    );

    ctx.contracts.eulDistributor = await (
      await ctx.factories.EulDistributor.deploy(
    ...
    console.log(
      `\nDeployed: Eul Distributor - ${ctx.contracts.eulDistributor.address}\n`
    );
  }
...
async function loadContracts(
  ...
  for (let name of Object.keys(addressManifest)) {
    ...
    if (name === "swapRouterV3") {
      ...
      ctx.contracts.swapRouterV3 = new ethers.Contract(
        addressManifest.swapRouterV3,
        abi,
        ethers.provider
      );
      continue;
    ...
    if (name === "swapRouter02") {
      ...
      ctx.contracts.swapRouter02 = new ethers.Contract(
        addressManifest.swapRouter02,
        abi,
        ethers.provider
      );
      continue;
    ...
    ctx.contracts[name] = await ethers.getContractAt(
      contractName,
      addressManifest[name]
    );
  }
  ...
  for (let name of Object.keys(addressManifest.modules)) {
    ctx.contracts.modules[name] = await ethers.getContractAt(
      instanceToContractName(name),
      addressManifest.modules[name]
    );
  }
  ...
  if (addressManifest.swapHandlers) {
    for (let name of Object.keys(addressManifest.swapHandlers)) {
      ctx.contracts.swapHandlers[name] = await ethers.getContractAt(
        instanceToContractName(name),
        addressManifest.swapHandlers[name]
      );
    }
  }
  ...
  if (ctx.tokenSetup.testing) {
    for (let tok of Object.keys(addressManifest.tokens)) {
      ctx.contracts.tokens[tok] = await ethers.getContractAt(
        "TestERC20",
        addressManifest.tokens[tok]
      );
...
async function getTaskCtx(tokenSetupName) {
  if (!tokenSetupName) {
    tokenSetupName =
      hre.network.name === "localhost" ? "testing" : hre.network.name;
  }

  let filename =
    hre.network.name === "localhost" && tokenSetupName === "testing"
...
class TestSet {
  ...
  run() {
    ...
    let fixture = fixtureFactory(
      this.args.fixture || "testing",
      this.args.forkAtBlock
    );
  ...
  async _runTest(spec, fixture) {
    if (spec.forkAtBlock)
      fixture = fixtureFactory("mainnet-fork", spec.forkAtBlock);
    let ctx = await loadFixture(fixture);

    let actions = [{ action: "checkpointTime" }];

    if (this.args.preActions)
      actions = actions.concat(this.args.preActions(ctx));
    ...
    for (let action of actions) {
      ...
      try {
        ...
        if (action.expectError) {
          ...
          if (e.message !== "Transaction reverted without a reason string")
            throw `Expected revert without reason, but got "${e.message}"`;
        } else {
          throw e;
        }
      }
      ...
      let makeBN = (x) =>
        typeof x === "number" ? ethers.BigNumber.from(x) : x;
      ...
      if (action.expectError !== undefined && !err)
        throw `expected error "${action.expectError}" but no error was thrown`;

      if (
        (process.env.INVARIANTS &&
          (action.send || action.action === "jumpTimeAndMine")) ||
        action.invariants
      ) {
        let markets = ["TST", "TST2", "TST3", "TST6", "TST9"].map(
  ...
  async _runAction(spec, ctx, action) {
    if (process.env.VERBOSE)
      console.log(
        action.send || action.call || action.callStatic || action.action
      );

    // Helps flushing out non-deterministic tests that incorrectly depend on test run-times
    if (process.env.TEST_SLEEP) {
      if (action.send || action.action === "jumpTimeAndMine")
        await sleep(parseInt(process.env.TEST_SLEEP));
    }

    let reportGas = (result) => {
      let name = action.send || action.action;
      if (this.args.gas || spec.gas || action.gas || process.env.GAS)
        console.log(`GAS(${name}) : ${result.gasUsed}`);
    };

    if (typeof action === "function") action = { cb: action };
    let args = await Promise.all(
      (action.args || []).map(async (a) =>
        typeof a === "function" ? await a() : a
      )
    );
    ...
    if (action.send !== undefined) {
      ...
      let tx = await contract
        .connect(from)
        .functions[components[0]].apply(null, args);
      ...
      if (action.simulate) {
        try {
          await ctx.contracts.exec
            .connect(from)
            .callStatic.batchDispatchSimulate(
              items,
              action.deferLiquidityChecks || []
            );
        } catch (e) {
        ...
        let tx = await ctx.contracts.exec
          .connect(from)
          .batchDispatch(items, action.deferLiquidityChecks || []);
      ...
      await ctx.setTokenBalanceInStorage(
        action.token,
        action.for,
        action.amount,
        action.slot
      ...
      await ctx.doUniswapSwap(
        action.from || ctx.wallet,
        action.tok,
        action.dir,
        action.amount,
        action.priceLimit
      ...
      await ctx.setIRM(
        ctx.contracts.tokens[action.underlying].address,
        moduleIds[action.irm],
        resetParams
      );
      ...
      ctx.contracts.modules.testModule = await (
        await ctx.factories.TestModule.deploy(action.id)
      ).deployed();
      await (
        await ctx.contracts.installer
          .connect(ctx.wallet)
          .installModules([ctx.contracts.modules.testModule.address])
      ).wait();
      ctx.contracts.testModule = await ethers.getContractAt(
        "TestModule",
        await ctx.contracts.euler.moduleIdToProxy(action.id)
      );
      ...
      if (ctx.tokenSetup.testing && ctx.tokenSetup.testing.forkTokens) {
        ...
        permitType =
          ctx.tokenSetup.testing.forkTokens[action.token].permit.type;
        permitDomain =
          ctx.tokenSetup.testing.forkTokens[action.token].permit.domain;
      } else {
...
function getSubAccount(primary, subAccountId) {
  if (parseInt(subAccountId) !== subAccountId || subAccountId > 256)
    throw `invalid subAccountId: ${subAccountId}`;
  return ethers.utils.hexZeroPad(
    ethers.BigNumber.from(primary).xor(subAccountId),
    20
  );
}
...
function equals(val, expected, tolerance) {
  if (typeof val === "number") {
    ...
    if (difference > tolerance) {
      ...
      throw Error(
        `equals failure: ${val} was not ${expected}${formattedTolerance}`
      );
    }
  } else {
    ...
    if (typeof expected === "number" || typeof expected === "string")
      expected = ethers.utils.parseEther("" + expected);
    if (typeof tolerance === "number" || typeof tolerance === "string")
      tolerance = ethers.utils.parseEther("" + tolerance);
    ...
    if (difference.gt(tolerance)) {
      let formattedTolerance = "";
      if (!tolerance.eq(0))
        formattedTolerance = ` +/- ${ethers.utils.formatEther(tolerance)}`;
      throw Error(
        `equals failure: ${ethers.utils.formatEther(
          val
        )} was not ${ethers.utils.formatEther(expected)}${formattedTolerance}`
      );
    }
  }
...
let taskUtils = {
  ...
  lookupAddress: async (ctx, addr) => {
    ...
    if (addr === "ref")
      return ctx.tokenSetup.riskManagerSettings.referenceAsset;
  ...
  lookupToken: async (ctx, sym) => {
    ...
    if (sym.startsWith("0x"))
      return await ethers.getContractAt("TestERC20", sym);
